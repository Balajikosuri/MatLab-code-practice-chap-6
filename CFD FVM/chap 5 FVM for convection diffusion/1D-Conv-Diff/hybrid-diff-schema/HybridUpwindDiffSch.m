function RunUpwindDiffSchConvectionDiffusion(varargin)
    ...
    [A, B] = HUDSTriDiagonalCoeffMatrix('N', N, ...
                                    'Diffusion', Gamma / dx, ...
                                    'Convection', rho * U, ...
                                    'PhiLeft', phi_A, ...
                                    'PhiRight', phi_B);
...
function RunCentrealDiffSchConvectionDiffusion(varargin)
    ...
    % Generate Tridiagonal Matrix
    [A, B] = CDSTriDiagonalCoeffMatrix('N', N, ...
                                    'Diffusion', Gamma / dx, ...
                                    'Convection', rho * U, ...
                                    'PhiLeft', phi_A, ...
                                    'PhiRight', phi_B);
...
for i = 1:length(U_values)
    for j = 1:length(N_values)
        ...
        if abs(Pe)<2
            fprintf(repmat('-', 1, 100)); fprintf('\n');
            fprintf('since Pe < 2(i.e %d) .So,(CDS) Central Differencing Method \n',Pe)
            RunCentrealDiffSchConvectionDiffusion('U', U, 'N', N, 'L',L, 'Gamma', Gamma, 'rho', rho, 'phi_A', phi_A, 'phi_B', phi_B);
        elseif abs(Pe)>=2
            fprintf(repmat('-', 1, 100)); fprintf('\n');
            fprintf('since Pe >= 2(i.e %d).So,(UDS) Upwind Differencing Method \n',Pe)
